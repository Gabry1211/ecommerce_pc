...
<div style="padding: 20px; max-width: 1200px; margin: 0 auto;">
    ...
    <h2>Gestione I Tuoi Prodotti</h2>
    ...
    <div style="background: white; padding: 20px; border: 1px solid #ddd; border-radius: 8px; margin-bottom: 30px;">
        <%
            ...
            String title = (pEdit != null) ? "Modifica Prodotto ID: " + pEdit.getIdProdotto() : "Aggiungi Nuovo Prodotto nel Tuo Negozio";
        %>
        <h3><%= title %></h3>
        <form action="<%=request.getContextPath()%>/VenditoreControl" method="post" enctype="multipart/form-data" style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
            ...
            <div class="form-group">
                ...
                <input type="text" name="tipo" value="<%= (pEdit != null) ? pEdit.getTipo() : "" %>" required placeholder="es. Monitor, PC Desktop, RAM">
            </div>
            <div class="form-group">
            ...
            <div style="grid-column: span 2; display: flex; gap: 10px;">
                <button type="submit" style="flex: 1; background: #ffd814; border: 1px solid #fcd200; padding: 10px; border-radius: 4px; cursor: pointer; font-weight: bold;">
                    <%= (pEdit != null) ? "Salva Modifiche" : "Metti in Vendita" %>
                </button>
                <% if (pEdit != null) { %>
                    <a href="<%=request.getContextPath()%>/VenditoreControl" style="flex: 1; text-align: center; background: #e7e9ec; border: 1px solid #adb1b8; padding: 10px; border-radius: 4px; text-decoration: none; color: black; font-size: 13px;">Annulla</a>
                <% } %>
            </div>
        </form>
    </div>
    ...
    <div style="background: white; padding: 20px; border: 1px solid #ddd; border-radius: 8px;">
        <h3>I Tuoi Articoli in Vendita</h3>
        <table style="width: 100%; border-collapse: collapse; margin-top: 15px;">
            ...
            <tbody>
                ...
                %>
                    <tr>
                        ...
                        <td style="padding: 10px; border: 1px solid #ddd; display: flex; gap: 10px;">
                            <a href="<%=request.getContextPath()%>/VenditoreControl?action=editProductLoad&id=<%= p.getIdProdotto() %>" style="color: #007185; text-decoration: none; font-weight: bold;">Modifica</a>
                            <a href="<%=request.getContextPath()%>/VenditoreControl?action=deleteProduct&id=<%= p.getIdProdotto() %>&token=<%= (String) session.getAttribute("token") %>" style="color: #c40000; text-decoration: none; font-weight: bold;" onclick="return confirm('Sicuro di voler rimuovere questo prodotto dalla vendita?')">Rimuovi</a>
                        </td>
                    </tr>
                <%
                        }
                ...
                %>
                    <tr><td colspan="6" style="padding: 20px; text-align: center;">Non hai ancora aggiunto prodotti al tuo catalogo.</td></tr>
                <%
                    }
                %>
            </tbody>
